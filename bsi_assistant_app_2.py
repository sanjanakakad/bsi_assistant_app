...
ASSISTANT_ID = _config("OPENAI_ASSISTANT_ID")
...
if arch != "-- None --":
    ...
    if st.button("💾 Save Expert Changes"):
        ...
        if save_json(EXPERT_STORE_FILE, expert_store):
            st.success("Expert changes saved.")
        else:
            st.warning(
                "Could not write to disk (common on Streamlit Cloud). "
                "Use **Download Matrix (CSV)** to keep your changes."
            )
